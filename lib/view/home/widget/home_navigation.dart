...
class HomeNavigationState extends State<HomeNavigation> {
  ...
  Widget _buidlButtonGroup() {
    return ListView(
      ...
      children: [
        navigatorButton(account.name, icon: Icons.sync_outlined, route: AccountRoutes.list),
        navigatorButton(
          "交易类型",
          icon: Icons.settings_outlined,
          route: TransactionCategoryRoutes.setting,
        ),
        navigatorButton("导入账单", icon: Icons.upload_outlined, route: TransactionImportRoutes.home),
        navigatorButton("导出账单", icon: Icons.download_outlined, route: AccountRoutes.list)
      ],
    );
  }

  Widget navigatorButton(String title, {IconData? titleIcon, String? subTitle, IconData? icon, required String route}) {
